...
class RankingMetrics:
    ...
    def ndcg_at_k(labels: np.ndarray, scores: np.ndarray, k: int) -> float:
        ...
        dcg = sum(labels[idx] / np.log2(pos + 2) for pos, idx in enumerate(sorted_indices))

        # Ideal DCG
        ideal_indices = np.argsort(-labels)[:k]
        idcg = sum(labels[idx] / np.log2(pos + 2) for pos, idx in enumerate(ideal_indices))
...
class CoverageMetrics:
    ...
    def gini_diversity(item_counts: dict[str, int]) -> float:
        ...
        gini = sum((2 * i - n - 1) * count for i, count in enumerate(counts, 1)) / (n * total)
...
class ModelEvaluator:
    ...
    def evaluate(
        ...
        metrics["reliability_diagram"] = CalibrationMetrics.reliability_diagram(
            labels, scores_candidate
        )
        ...
        for gate_name, gate in gates.items():
            ...
            self.logger.info(
                f"  {icon} {gate_name}: {gate['value']:.4f} (threshold: {gate['threshold']:.4f})"
            )
