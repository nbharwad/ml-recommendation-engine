...
EVENT_LATENCY = Histogram("ingestion_latency_ms", "Ingestion latency", buckets=[0.5, 1, 2, 5, 10, 20])
...
class RateLimiter:
    ...
    def __init__(self, max_tokens: int = 50, refill_rate: float = 50.0):
...
class EventIngestionService:
    ...
    async def ingest_event(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        if not is_valid:
            EVENT_INGESTED.labels(event_type=event.get("event_type", "UNKNOWN"), status="rejected").inc()
        ...
        if not self.rate_limiter.allow(event["user_id"]):
            ...
            return {"event_id": event["event_id"], "status": "RATE_LIMITED", "error": "Rate limit exceeded"}
    ...
    async def _produce(self, event: dict[str, Any]):
        ...
        self.logger.debug("event_produced", event_id=event["event_id"], topic=self.config.kafka_topic)
